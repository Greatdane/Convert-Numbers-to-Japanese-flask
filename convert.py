...
romanji_dict = {".": "ten", "0": "zero", "1": "ichi", "2": "ni", "3": "san", "4": "yon", "5": "go", "6": "roku", "7": "nana",
...
key_dict = {"K" : kanji_dict, "H" : hiragana_dict, "R": romanji_dict}
...
def split_Point(split_num,dict_choice):
    ...
    # To account for small exception of small tsu when ending in jyuu in hiragana/romanji
    if split_num_a[-1] == "0" and split_num_a[-2] != "0" and dict_choice == "H":
        small_Tsu = web_convert(split_num_a,dict_choice)
        small_Tsu = small_Tsu[0:-1] + "っ"
        return small_Tsu + key_dict[dict_choice]["."] + split_num_b_end
    if split_num_a[-1] == "0" and split_num_a[-2] != "0" and dict_choice == "R":
        small_Tsu = web_convert(split_num_a,dict_choice)
        small_Tsu = small_Tsu[0:-1] + "t"
        return small_Tsu + key_dict[dict_choice]["."] + split_num_b_end
    #
    return web_convert(split_num_a,dict_choice) + " " + key_dict[dict_choice]["."] + split_num_b_end

def web_convert(convert_num, dict_choice):
    # remove commas
    convert_num = convert_num.replace(',','')
    # remove any leading zeroes
    while convert_num[0] == "0" and len(convert_num) > 1:
        convert_num = convert_num[1:]
    dict = key_dict[dict_choice]
    if "." in convert_num:
        web_result = split_Point(convert_num,dict_choice)
    else:
        web_result = convert(convert_num, dict)
    if key_dict[dict_choice] == romanji_dict:
        pass
    else:
        web_result = remove_spaces(web_result)
    return web_result

# test
# print(web_convert("20.5","K"))
